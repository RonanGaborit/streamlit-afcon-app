...
def passe(df, selected_team):
    ...
    kde_receipt = pitch.kdeplot(df_ball_receipt.x, df_ball_receipt.y, ax=axs['pitch'][1], levels=200, shade_lowest=True,
                                cut=100, cmap=flamingo_cmap, fill=True)
    ...
    axs['title'].text(0.5, 1.5, f'{team1}  Passes ', color='#F5F5DC', fontsize=25, va='center', ha='center',
                      path_effects=path_eff,
                      fontproperties=robotto_regular.prop)
...
def shot(df):
    ...
    fig, axs = pitch.grid(figheight=10, title_height=0.08, endnote_space=0,
                          ...
                          title_space=0, grid_height=0.82, endnote_height=0.05, grid_width=0.88)
    fig.set_facecolor(back)
    pitch.scatter(df_goal.x, df_goal.y, c='Green', ax=axs['pitch'], s=450, edgecolor="w", marker="*", alpha=.8,
                  label='Goal')
...
def pass_cross(df):
    ...
    pitch.lines(df.x, df.y, df.end_x, df.end_y, ax=axs['pitch'], capstyle='butt',
                # cut-off the line at the end-location.
    ...
    axs['title'].text(0.5, 0.6, 'Crosses', color='#F5F5DC', fontsize=35, va='center', ha='center',
                      path_effects=path_eff,
                      fontproperties=robotto_regular.prop)
    axs['endnote'].text(1, 0.85, '@SoccerbyNumber6', va='center', ha='right', fontsize=10,
                        fontproperties=robotto_regular.prop, color='#F5F5DC')
...
def transition(df):
    ...
    pitch.lines(df.x, df.y,
                ...
                alpha=ALPHA_PASS_LINE / NUM_GLOW_LINES,
                color=PASS_COLOR, comet=True, ax=axs['pitch'])
    axs['title'].text(0.5, 0.6, 'Forward passes', color='#F5F5DC', fontsize=35, va='center', ha='center',
                      path_effects=path_eff,
                      fontproperties=robotto_regular.prop)
    axs['endnote'].text(1, 0.85, '@SoccerbyNumber6', va='center', ha='right', fontsize=10,
                        fontproperties=robotto_regular.prop, color='#F5F5DC')
...
def pressure_heatmap(df, team_choice):
    ...
    axs['title'].text(0.5, 0.9, 'Pressure applied by', color='#F5F5DC', fontsize=30, va='center', ha='center',
                      path_effects=path_eff,
                      fontproperties=robotto_regular.prop)
...
def mistake(df):
    ...
    pitch.scatter(df_dispos.x, df_dispos.y, color="w", s=100, edgecolor="w", marker="o", alpha=.5,
                  ax=axs['pitch'][0][0])
    bin_statistic = pitch.bin_statistic(df_dispos.x, df_dispos.y, statistic='count', bins=(25, 25))
    bin_statistic['statistic'] = gaussian_filter(bin_statistic['statistic'], 1)
    pcm = pitch.heatmap(bin_statistic, ax=axs['pitch'][0][0], cmap='hot', edgecolors='#22312b',
                        label=f'{team} Clearances')
...
def player(df):
    ...
    fig, axs = pitch.grid(ncols=2, nrows=2, grid_height=0.6, title_height=0.05, axis=False,
                          endnote_height=0.04, title_space=0, endnote_space=0, grid_width=0.88, figheight=10)
    fig.set_facecolor(back)
    pitch.lines(passes.x, passes.y, passes.end_x, passes.end_y, ax=axs['pitch'][0][0], capstyle='butt',
                # cut-off the line at the end-location.
                linewidth=LINEWIDTH, color=PASS_COLOR)
    ...
    pitch.lines(carry.x, carry.y, carry.end_x, carry.end_y, ax=axs['pitch'][0][1], linewidth=0.9, color='Orange',
                comet=True)
    for i in range(1, NUM_GLOW_LINES + 1):
